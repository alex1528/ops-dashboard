...
- ✅ 凭据管理：每个目标独立的加密凭据存储 (AES-256-GCM)，查看凭据带 loading 反馈并以弹窗展示解密后的用户名/密码（支持一键复制）；编辑时预先获取凭据再打开弹窗，用户名/密码（星号显示）可靠回显；用户名为空而密码有值时同样正常存取；编辑时两字段均留空则不覆盖已存储值；解密失败时返回明确错误提示
