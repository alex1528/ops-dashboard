...
- ✅ 状态看板：展示所有目标资源的在线状态、响应时间；每张卡片底部显示模式标签（"自动登录"绿色 / "外链"蓝色），并提供独立的「打开链接 / 一键直达」按钮（自动登录模式走代理注入凭据，外链模式直接打开新标签页）；为避免误点开外部页面，**卡片本身不再绑定点击跳转**，所有跳转操作都需点击底部明确的图标按钮；卡片排序严格遵循管理后台拖拽调整后的分组顺序和组内资源顺序（groupSortOrder → sortOrder → createdAt 三级排序）；已登录用户（普通用户及管理员）可在每张卡片上点击「查看凭据」按钮，对已启用 SSH 的资源额外显示「SSH 终端」按钮（点击打开浏览器内 WebTerminal），未登录时功能按钮不显示；普通用户仅可见管理员授权的资源卡片
...
- ✅ 代理自动登录：已移除（凭据仅作为记录存储，不再自动填入目标系统登录表单）
- ✅ 反向代理网关：已移除
