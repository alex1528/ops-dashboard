...
- ✅ 状态看板：展示所有目标资源的在线状态、响应时间；卡片排序严格遵循管理后台拖拽调整后的分组顺序和组内资源顺序（groupSortOrder → sortOrder → createdAt 三级排序）；已登录用户（普通用户及管理员）可在每张卡片上点击「查看凭据」按钮，对已启用 SSH 的资源额外显示「SSH 终端」按钮，未登录时按钮不显示
