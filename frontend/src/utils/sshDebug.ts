...
//
// Zero runtime impact by default: logging is OFF unless enabled by either of
// the two layers below (localStorage wins, so a build-time default can always
// be overridden per-browser without rebuilding):
//
//   1. Build-time default — set VITE_SSH_DEBUG=1 in the repo-root .env.
//      Compose passes it as a Docker build arg into the frontend build stage,
//      so it is baked into the bundle. Changing it requires ./build.sh.
//   2. Runtime override — in the browser devtools console:
//        localStorage.setItem('sshDebug', '1')   // force ON  (then reload)
//        localStorage.setItem('sshDebug', '0')   // force OFF (then reload)
//        localStorage.removeItem('sshDebug')     // fall back to the build default
//
...
/**
 * True when SSH byte logging is enabled.
 *
 * Resolution order: localStorage.sshDebug ('1' → on, any other value → off)
 * takes precedence; when the key is absent, the build-time VITE_SSH_DEBUG
 * default applies. Result is cached, so a reload is required after changing it.
 */
export function isSshDebugEnabled(): boolean {
  if (cachedEnabled !== null) return cachedEnabled;
  const buildDefault = import.meta.env.VITE_SSH_DEBUG === '1';
  try {
    const stored = localStorage.getItem('sshDebug');
    cachedEnabled = stored === null ? buildDefault : stored === '1';
  } catch {
    // localStorage can throw in private/blocked-cookie contexts.
    cachedEnabled = buildDefault;
  }
